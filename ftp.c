                                   ...
                                   bool sizechecked)
{
  ...
  //Modified here *******
  int check1 = ((data->state.resume_from && !sizechecked) || ((data->state.resume_from > 0) && sizechecked)) if (check1)
  {
    ...
    /* => transfer as usual */
    int check2 = (data->state.resume_from < 0);
    if (check2)
    {
    ...
    //######################EDITED PART###########################
    seekerr = changeSeekerr(conn, seekerr);

    int check3 = (seekerr != CURL_SEEKFUNC_OK);
    if (check3)
    {
      int check4 = (seekerr != CURL_SEEKFUNC_CANTSEEK);
      if (check4)
      {
...
static int changeSeekerr(struct connectdata *conn, int seekerr, )
{
  if (conn->seek_func)
  {
...
static int doLoopCase(curl_off_t passed, struct connectdata *conn)
{
  do
  {
    size_t readthisamountnow =
        (data->state.resume_from - passed > CURL_OFF_T_C(BUFSIZE)) ? BUFSIZE : curlx_sotouz(data->state.resume_from - passed);
    ...
    passed += actuallyread;
    int check5 = ((actuallyread == 0) || (actuallyread > readthisamountnow));
    if (check5)
    {
...
CURLcode ftp_state_pasv_resp(struct connectdata *conn, int ftpcode);
